...
export default class WHIPClient {
  ...
  constructor(private endpoint: string, private incomingStream: MediaStream) {
    /**
    ...
    // 	})
    // 	.catch(console.error);
    this.accessLocalMediaSources(this.incomingStream)
  }
   ...
   */
  private async accessLocalMediaSources(stream: MediaStream) {
    if(!stream) return;
